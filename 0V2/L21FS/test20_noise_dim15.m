...
for i=1:DataNum
    ...
    if ~exist([num2str(rate),'Data_dim15'],'dir') 
        mkdir([num2str(rate),'Data_dim15']);
    end 
    savePath=[num2str(rate),'Data_dim15/Data_',DataName{i},'.mat'];
    
    if outratio >0
        savetemp=[num2str(rate),'Data_dim15_',num2str(outratio),'Noise'];
