...
for i=1:DataNum
    ...
    if ~exist([num2str(rate),'Data_dim40'],'dir') 
        mkdir([num2str(rate),'Data_dim40']);
    end 
    savePath=[num2str(rate),'Data_dim40/Data_',DataName{i},'.mat'];
    
    if outratio >0
        savetemp=[num2str(rate),'Data_dim40_',num2str(outratio),'Noise'];
